...
export class StaffComplaintsService extends BaseComplaintsService {
  ...
  async lockComplaint(staff: InternalUserEntity, id: number): Promise<ComplaintEntity> {
    ...
    const releasedCount = await this.your-bucket-nameRepo.releaseAllLocksForUser(staff.id, ComplaintLockerRole.INTERNAL_USER);

    // Invalidate cache for released locks and new lock
    if (releasedCount > 0) {
      await this.cacheInvalidation.invalidateComplaintCaches();
    }

    const lockedComplaint = await this.your-bucket-nameRepo.lock(complaint.id, staff.id, ComplaintLockerRole.INTERNAL_USER);
